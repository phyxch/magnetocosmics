...
C
      SUBROUTINE T89C_(IOPT,PARMOD,PS,X,Y,Z,BX,BY,BZ)
C
C
...
C
      SUBROUTINE T89C_BOBERG_(IOPT,PARMOD,PS,X,Y,Z,BX,BY,BZ)
C
C
C   COMPUTES GSM COMPONENTS OF THE MAGNETIC FIELD PRODUCED BY EXTRA-
C   TERRESTRIAL CURRENT SYSTEMS IN THE GEOMAGNETOSPHERE. THE MODEL IS
...
C
         ENDIF
c       BOBERG et al formula	 
	 A(5)=-10220. + 408.5 * PARMOD(2) 
C
        XI(1)=X
        XI(2)=Y
        XI(3)=Z
...
C
C
          SUBROUTINE  T89_BOBERG_(ID, A, XI, F, DER)
C
C        ***  N.A. Tsyganenko ***  8-10.12.1991  ***
C
C      Calculates dependent model variables and their deriva-
